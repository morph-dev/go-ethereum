...
type generateParams struct {
	...
	chunks      []*types.ChunkTransactions // List of pre-built chunks to use for creation
}
...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	if !genParam.noTxs {
		...
		preSelectedTx := make(types.Transactions, 0)
		for _, chunk := range genParam.chunks {
			preSelectedTx = append(preSelectedTx, chunk.Transactions...)
		}

		err := miner.fillTransactions(interrupt, work, preSelectedTx)
	...
	if miner.config.Chunks {
		body.Chunks, err = types.SplitIntoChunks(work.receipts, genParam.chunks)
		if err != nil {
			return &newPayloadResult{err: err}
		}
	}
...
func (miner *Miner) fillTransactions(interrupt *atomic.Int32, env *environment, preSelectedTx types.Transactions) error {
	miner.confMu.RLock()
	...
	filter := txpool.PendingFilter{
		...
		ExcludedTxHashes: preSelectedTx.HashSet(),
	}
	...
	// Fill the block with preSelectedTransactions
	if env.gasPool == nil {
		env.gasPool = new(core.GasPool).AddGas(env.header.GasLimit)
	}
	for _, tx := range preSelectedTx {
		env.state.SetTxContext(tx.Hash(), env.tcount)
